...
import com.ceb.bank.hfield.HAshmxField;
...
public class AshmxLxScanner extends AbstractLxScanner {
	private static final Logger log = Logger.getLogger(AshmxLxScanner.class);
	private ILxVerifier verifier = new LxVerifierImpl();

	public AshmxLxScanner() {
		super(HBaseQueryConstants.TABLE_ASHMX);
	}
	...
	protected LxContext parse(Result result) throws IOException {
		...
		Map<String, byte[]> qualifiers = HAshmxField.getLxQualifiers();
