...
public class BkhhdIndexScanner extends AbstractQuery<String>{
	private static final Logger log = Logger.getLogger(BkhhdScanner.class);

	public BkhhdIndexScanner(String tableName) {
		super(tableName);
	}
	...
	public List<Item9019> query(PybjyEO record, List<RowkeyContext> list) throws Exception {
		...
		for (RowkeyContext info : list) {
			...
			List<String> tmpList = this.scan(scanner, info.getPageCount());
	...
	protected String parse(Result result) throws IOException {
		String rowkey = "";
		Map<String, byte[]> qualifiers = BkhhdIndexScanner.getMxQualifiers();
