...
import com.ceb.bank.query.output.txt.TxtGenerator0772;
import com.ceb.bank.query.scanner.AsdmxPdfScanner;
import com.ceb.bank.query.scanner.AsdmxTxtScanner;
...
public class Output0772 extends AbstractOutput {
	private static final Logger log = Logger.getLogger(Output0772.class);
	...
	private String callTxtMaker(final PybjyEO record, final Output0771Context ctx, int pageSize) throws Exception {
		...
		final TxtGenerator0772 generator = new TxtGenerator0772();
		if (!ctx.isQueryExist()) {
			ctx.setPageNum(1);
		...
		for (int i = 0; i < zhanghNum; i++) {
			...
			AsdmxTxtScanner queryMxScanner = new AsdmxTxtScanner() {
				@Override
