...
public class SkyHTable implements HTableInterface, Closeable {
	private static final Log LOG = LogFactory.getLog(SkyHTable.class);
	 ...
	 */
	public SkyHTable(final String tableName) throws IOException {
	 ...
	 * @see #HTable(Configuration, String)
	 */
	public SkyHTable(final byte[] tableName) throws IOException {
	 ...
	 *             if a remote or network exception occurs
	 */
	public SkyHTable(Configuration conf, final String tableName) throws IOException {
	 ...
	 *             if a remote or network exception occurs
	 */
	public SkyHTable(Configuration conf, final byte[] tableName) throws IOException {
		...
		this.pool = new ThreadPoolExecutor(1, maxThreads, keepAliveTime, TimeUnit.SECONDS, new SynchronousQueue<Runnable>(), Threads.newDaemonThreadFactory("hbase-table"));
	 ...
	 *             if a remote or network exception occurs
	 */
	public SkyHTable(final byte[] tableName, final HConnection connection, final ExecutorService pool) throws IOException {
	...
	private void finishSetup() throws IOException {
		...
		this.operationTimeout = HTableDescriptor.isMetaTable(tableName) ? HConstants.DEFAULT_HBASE_CLIENT_OPERATION_TIMEOUT : this.configuration.getInt(
	 ...
	 */
	// TODO(tsuna): Remove this. Unit tests shouldn't require public
	// helpers.
	...
	public Pair<byte[][], byte[][]> getStartEndKeys() throws IOException {
		...
		MetaScannerVisitor visitor = new MetaScannerVisitor() {
			public boolean processRow(Result rowResult) throws IOException {
				...
				if (bytes == null) {
					LOG.warn("Null " + HConstants.REGIONINFO_QUALIFIER_STR + " cell in " + rowResult);
					return true;
				}
		...
		return new Pair<byte[][], byte[][]>(startKeyList.toArray(new byte[startKeyList.size()][]), endKeyList.toArray(new byte[endKeyList.size()][]));
	}
	...
	public Result getRowOrBefore(final byte[] row, final byte[] family) throws IOException {
		return connection.getRegionServerWithRetries(new ServerCallable<Result>(connection, tableName, row, operationTimeout) {
	...
	public Result get(final Get get) throws IOException {
		return connection.getRegionServerWithRetries(new ServerCallable<Result>(connection, tableName, get.getRow(), operationTimeout) {
			public Result call() throws IOException {
	 ...
	 */
	@Override
	public synchronized void batch(final List<Row> actions, final Object[] results) throws InterruptedException, IOException {
	...
	public void delete(final Delete delete) throws IOException {
		connection.getRegionServerWithRetries(new ServerCallable<Boolean>(connection, tableName, delete.getRow(), operationTimeout) {
	...
	public void delete(final List<Delete> deletes) throws IOException {
		...
		try {
			...
			// results are returned in the same order as the requests in
			// list
	...
	/**
	 * 根据输入的list返回List中成功的元素
	 * 
	 * @param puts
	 * @return
	 * @throws IOException
	 */
	public List<Put> putWithResult(final List<Put> puts) throws IOException {
		return doPutWithResult(puts);
	}
	...
	private List<Put> doPutWithResult(final List<Put> puts) throws IOException {
		for (Put put : puts) {
			validatePut(put);
			writeBuffer.add(put);
			currentWriteBufferSize += put.heapSize();
		...
		LOG.debug("doPutWithResult  中buffer的大小是：" + writeBuffer.size());
		return flushCommitsWithResult();
	}
	...
	public Result increment(final Increment increment) throws IOException {
		...
		return connection.getRegionServerWithRetries(new ServerCallable<Result>(connection, tableName, increment.getRow(), operationTimeout) {
			public Result call() throws IOException {
	 ...
	 */
	@Override
	public long incrementColumnValue(final byte[] row, final byte[] family, final byte[] qualifier, final long amount) throws IOException {
	 ...
	 */
	@Override
	public long incrementColumnValue(final byte[] row, final byte[] family, final byte[] qualifier, final long amount, final boolean writeToWAL) throws IOException {
		...
		return connection.getRegionServerWithRetries(new ServerCallable<Long>(connection, tableName, row, operationTimeout) {
			public Long call() throws IOException {
				return server.incrementColumnValue(location.getRegionInfo().getRegionName(), row, family, qualifier, amount, writeToWAL);
			}
		});
	}
	...
	public boolean checkAndPut(final byte[] row, final byte[] family, final byte[] qualifier, final byte[] value, final Put put) throws IOException {
		return connection.getRegionServerWithRetries(new ServerCallable<Boolean>(connection, tableName, row, operationTimeout) {
			public Boolean call() throws IOException {
				return server.checkAndPut(location.getRegionInfo().getRegionName(), row, family, qualifier, value, put) ? Boolean.TRUE : Boolean.FALSE;
			}
		});
	}
	 ...
	 */
	@Override
	public boolean checkAndDelete(final byte[] row, final byte[] family, final byte[] qualifier, final byte[] value, final Delete delete) throws IOException {
		return connection.getRegionServerWithRetries(new ServerCallable<Boolean>(connection, tableName, row, operationTimeout) {
			public Boolean call() throws IOException {
				return server.checkAndDelete(location.getRegionInfo().getRegionName(), row, family, qualifier, value, delete) ? Boolean.TRUE : Boolean.FALSE;
			}
		});
	}
	...
	public void flushCommits() throws IOException {
		try {
			...
			try {
				...
				// mutate list so that it is empty for complete success, or
				// contains
				// only failed records results are returned in the same
				// order as the
				// requests in list walk the list backwards, so we can
				// remove from list
	...
	public List<Put> flushCommitsWithResult() throws IOException {
		try {
			Object[] results = new Object[writeBuffer.size()];
			try {
				this.connection.processBatch(writeBuffer, tableName, pool, results);
	...
	public RowLock lockRow(final byte[] row) throws IOException {
		return connection.getRegionServerWithRetries(new ServerCallable<RowLock>(connection, tableName, row, operationTimeout) {
	...
	public void unlockRow(final RowLock rl) throws IOException {
		connection.getRegionServerWithRetries(new ServerCallable<Boolean>(connection, tableName, rl.getRow(), operationTimeout) {
			public Boolean call() throws IOException {
	 ...
	 * multiple regions in a table, this scanner will iterate through them all.
	 */
	protected class ClientScanner implements ResultScanner {
		private final Log CLIENT_LOG = LogFactory.getLog(this.getClass());
		// HEADSUP: The scan internal start row can change as we move
		// through table.
		private Scan scan;
		...
		protected ClientScanner(final Scan scan) {
			if (CLIENT_LOG.isDebugEnabled()) {
				CLIENT_LOG.debug("Creating scanner over " + Bytes.toString(getTableName()) + " starting at key '" + Bytes.toStringBinary(scan.getStartRow()) + "'");
			}
			this.scan = scan;
			this.lastNext = System.currentTimeMillis();

			// Use the caching from the Scan. If not set, use the default
			// cache setting for this table.
			if (this.scan.getCaching() > 0) {
				this.caching = this.scan.getCaching();
			} else {
				this.caching = SkyHTable.this.scannerCaching;
			}

			// Removed filter validation. We have a new format now, only one
			// of all
			// the current filters has a validate() method. We can add it
			// back,
			// need to decide on what we're going to do re: filter redesign.
			// Need, at the least, to break up family from qualifier as
			// separate
			// checks, I think it's important server-side filters are
			// optimal in that
			// respect.
		}
		...
		private boolean checkScanStopRow(final byte[] endKey) {
			if (this.scan.getStopRow().length > 0) {
				...
				if (cmp <= 0) {
					// stopRow <= endKey (endKey is equals to or larger than
					// stopRow)
					// This is a stop.
					return true;
				}
		...
		/*
		 * Gets a scanner for the next region. If this.currentRegion != null,
		 * then we will move to the endrow of this.currentRegion. Else we will
		 * get scanner at the scan.getStartRow(). We will go no further, just
		 * tidy up outstanding scanners, if <code>currentRegion != null</code>
		 * and <code>done</code> is true.
		 * 
		 * @param nbRows
		 * 
		...
		private boolean nextScanner(int nbRows, final boolean done) throws IOException {
			...
			// if we're at end of table, close and return false to stop
			// iterating
			...
			if (CLIENT_LOG.isDebugEnabled()) {
				CLIENT_LOG.debug("Advancing internal scanner to startKey at '" + Bytes.toStringBinary(localStartKey) + "'");
			}
			try {
		...
		protected ScannerCallable getScannerCallable(byte[] localStartKey, int nbRows) {
			...
			ScannerCallable s = new ScannerCallable(getConnection(), getTableName(), scan);
			s.setCaching(nbRows);
			return s;
		}

		public Result next() throws IOException {
			// If the scanner is closed but there is some rows left in the
			// cache,
			...
			if (cache.size() == 0) {
				...
				// We need to reset it if it's a new callable that was
				// created
				// with a countdown in nextScanner
				callable.setCaching(this.caching);
				// This flag is set when we want to skip the result
				// returned. We do
				...
				do {
					try {
						if (skipFirst) {
							// Skip only the first row (which was the last
							// row of the last
						...
						// Server returns a null values if scanning is to
						// stop. Else,
						// returns an empty array if scanning is to go on
						// and we've just
						...
						if (e instanceof UnknownScannerException) {
							...
							// If we are over the timeout, throw this
							// exception to the client
							// Else, it's because the region moved and we
							// used the old id
							// against the new region server; reset the
							// scanner.
							if (timeout < System.currentTimeMillis()) {
								long elapsed = System.currentTimeMillis() - lastNext;
								// ScannerTimeoutException ex = new
								// ScannerTimeoutException(
								// elapsed +
								// "ms passed since the last invocation, " +
								// "timeout is currently set to " +
								// scannerTimeout);
								// ex.initCause(e);
								try {
									throw new TimeoutException(elapsed + "ms passed since the last invocation, " + "timeout is currently set to " + scannerTimeout);
						...
						// Else, its signal from depths of ScannerCallable
						// that we got an
						// NSRE on a next and that we need to reset the
						// scanner.
						if (this.lastResult != null) {
							this.scan.setStartRow(this.lastResult.getRow());
							// Skip first row returned. We already let it
							// out on previous
							// invocation.
							skipFirst = true;
						}
					...
					// Values == null means server-side filter has
					// determined we must STOP
		...
		/**
		 * Get <param>nbRows</param> rows. How many RPCs are made is determined
		 * by the {@link Scan#setCaching(int)} setting (or
		 * hbase.client.scanner.caching in hbase-site.xml).
		 * 
		 * @param nbRows
		 *            number of rows to return
		 * @return Between zero and <param>nbRows</param> RowResults. Scan is
		 *         done if returned array is of zero-length (We never return
		 *         null).
		 * @throws IOException
		 */
		...
		public void close() {
			if (callable != null) {
				...
				try {
					...
					// We used to catch this error, interpret, and rethrow.
					// However, we
					// have since decided that it's not nice for a scanner's
					// close to
					// throw exceptions. Chances are it was just an
					// UnknownScanner
		...
		public Iterator<Result> iterator() {
			return new Iterator<Result>() {
				...
				// return true if there is another item pending, false if
				// there isn't.
				// this method is where the actual advancing takes place,
				// but you need
				// to call next() to consume it. hasNext() will only advance
				// if there
				...
				// get the pending next item and advance the iterator.
				// returns null if
				// there is no next item.
				public Result next() {
					// since hasNext() does the real advancing, we call this
					// to determine
					...
					// if we get to here, then hasNext() has given us an
					// item to return.
	 ...
	 * @throws IOException
	 */
	public static void setRegionCachePrefetch(final Configuration conf, final byte[] tableName, final boolean enable) throws IOException {
	...
	public <T extends CoprocessorProtocol> T coprocessorProxy(Class<T> protocol, byte[] row) {
		return (T) Proxy.newProxyInstance(this.getClass().getClassLoader(), new Class[] { protocol }, new ExecRPCInvoker(configuration, connection, protocol, tableName, row));
	}
	 ...
	 */
	@Override
	public <T extends CoprocessorProtocol, R> Map<byte[], R> coprocessorExec(Class<T> protocol, byte[] startKey, byte[] endKey, Batch.Call<T, R> callable) throws IOException,
			Throwable {
